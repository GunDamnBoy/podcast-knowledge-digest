...
**排查時務必看日誌的時間戳，不是只看內容（2026-08-03 事故）。** 08-03 的日誌開頭是 `[07:00:01]` 而非 `[01:00:0x]`，比日報的 03:00 還晚四小時。**已確認 plist 的 `StartCalendarInterval` 本來就是 01:00，所以這不是排程設定被改掉**，而是 launchd 在 01:00 那一刻沒能執行、延到 07:00 才補跑（最可能是機器當時睡眠或關機——`StartCalendarInterval` 不會喚醒 Mac，錯過的排程要等下次醒來才補）。

後果一樣嚴重：日報 cron 改成 03:00 後，只要 podfetch 當天延後補跑，日報就會在它之前執行，於是**讀不到當天目錄、也讀不到當天日誌，被上面第 3 點誤判成「podfetch 失效」而掉進已死的 YouTube 退援路徑，產出降級內容並誤報故障**。08-03 沒有爆掉純粹是因為日報那次仍跑在舊的 09:00。
