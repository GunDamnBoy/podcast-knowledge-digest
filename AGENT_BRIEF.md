...
**每天**（含週末）抓新發布的集數。**視窗以 `last_run_utc` 為準**（見本節「四個不要改的設計」第 1 項），往前重疊 30 分鐘、上限 72 小時；**正常情況下大約是 24–26 小時，但那是結果不是設定**。重疊的代價是**相鄰兩天會重複命中同一集**。
...
- 被略過的那一集**不另外寫一集**，改在保留那集的 `meta` 加一欄 `{"k":"同源推送","v":"同日亦推送於 Odd Lots"}`，`episodeCount` 與 `index.json` 的 `shows` 都按**去重後**計算。
- 在回報中列出被合併的集數。
