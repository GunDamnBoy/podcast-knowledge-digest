...
- **事故敘事下推：只搬了兩塊、約 500 字元，並訂正了前一天的估計。** 前一日估「第 1–7 節有 43% 是事故敘事」，那是**行層級的上界**；實際切分九個候選區塊只有 **2,529 字元**——**佔改動前全檔的 6.8%；若與 43% 對齊分母（第 1–7 節）則是 8.1%**（**那次訂正一口氣換了兩個分母：範圍由第 1–7 節改成全檔、單位由行改成字元，而只說明了後者**），**其中七塊逐塊判斷後判定該留**——113 秒預告佔滿當天兩集、跳針 28,122 個「I」讓完整度反向、wpm 不決定 `DEGRADED`、503 併進 `EXHAUSTED` 的事故鏈、時間軸雙訊號的設計理由，**這些細節本身就是規則的動機，刪了規則就變成沒來由的規定**。只把「維護者才用得到的 meta 紀律」與「逐項的事後數字」移進 `MAINTENANCE.md` 第 7 節。
- **刪掉五處會過期的抄本（本檔 −771 字元）。** 以 `writing-for-agents` 六槓桿稽核本檔與排程 `SKILL.md` 後，第一批只做「刪除且互不相干」的項目：①本檔第 5 行的工具呼叫預算數值「（目標 2 次／上限 5 次）」——08-17 已改為 R+1／R+3，**這裡是唯一殘留的舊值，而且與本節同日條目直接打架**，改為只留規則名稱、數值以 `SKILL.md` 為唯一來源；②排程 `SKILL.md` 開頭「舊版寫的『約 45K 字元會進主 context 一次，這是已接受成本』已不適用，不要照舊做」整句刪——**為了宣告一條規則作廢而把它完整寫進 context，等於每天提醒執行者那個作廢的做法存在**；③第 2 節退援順序「只有四層」後的訂正括號（「FT 曾被列成獨立的第 3 層⋯」）刪——同型，且正解「第 1 層的特例：FT 專用流程」下一段已完整寫明；④第 1 節的 23 組 AppleID 清單刪，改為指向 `~/.podfetch/shows.json`——**本檔原本自承那份清單「是給人看的、不一致時以 shows.json 為準」，等於一份無機械把關的過期候選**（`healthcheck.py` 不讀它，只 regex 比對「N 檔」）；退援路徑要用 AppleID 的地方已在 `SKILL.md` 第 1 步就地註明讀 `shows.json`；⑤第 3 行「任何一個新的 Cowork 對話讀了這份，就能完整重現整套系統的規格面」刪——純敘述，不改變任何行為；同句的語言與讀者設定保留。
  **為什麼這五處排在最前面**：全部是刪除、彼此無依賴，任一處改壞的最壞後果都只是少一句話，適合當第一批。**下一批要處理的是本檔一次 `Read` 讀不完（實測 29,112 token > 25,000 上限）**——第 0 步「完整讀過 `AGENT_BRIEF.md`」在機制上做不到單次完成，這與本日 `Read` 上限那條是同一個機制，當時只套用到逐字稿、沒有回頭套用到本檔自己。
