...
2. **連上了但沒有今天的目錄** → 讀 `~/.podfetch/logs/<今天>.log`。停在異常處就是 podfetch 失效。也可看 `state.json` 的 `last_run_utc` 是否已推進到今天。
   ...
   > - 有集數落在視窗內卻沒被 podfetch 收到 → **假零集**。此時 `last_run_utc` 已經被推進、缺口會變成永久，處理方式是把它回捲到前一次的值再重跑 podfetch，並在回報中明講。
   >
...
- **FT 入口第二次寫錯，這次連「已更正的版本」也是錯的。** 08-08 把入口從 `ft.com/unhedged` 改成 `ft.com/the-economics-show`，但後者只列 The Economics Show 自己的集數、**一樣不含 Unhedged**。可用的入口是 FT 站內搜尋 `ft.com/search?q="<原文標題>"`，而且**每集有兩個頁面，要取標題帶 `Transcript:` 前綴的那一個**——另一個集數頁正文僅約 2,700 字元，很容易被誤判成「付費牆擋住」而白白退層。第 1、2 節已同步。
...
- **三處與實況脫鉤的敘述一併訂正**：①ILTB 語速兩處打架（第 1 節寫 122，第 2 節與 `shows.json` 都是 140）；②檔名慣例把窗口寫成「台北 01:00 至 03:00」，實際**兩端都由 podfetch 決定**（實測 00:30 → 隔日 01:00、24.5 小時，終點是 podfetch 的執行時刻而非日報的）；③本檔開頭宣稱「讀了這份就能完整重現整套系統」，但三條成本紀律只存在於 SKILL.md，已改為明確指路而非默默不提。
...
>
> **更早的變更紀錄（2026-08-02 至 08-14）已歸檔至 `MAINTENANCE.md` 第 11 節。**
