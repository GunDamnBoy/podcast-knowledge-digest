...
- **回歸複驗抓到 6 處，其中 3 處是今天這輪改動自己製造的，全數已修。** 最要緊的一處是**「假零集要回捲 `last_run_utc`」這句舊指示，在同一段落裡與新行為直接打架**——0 集已經不推進它了，回捲反而會把視窗起點往回多推一格。brief 第 2 節與 SKILL.md 第 1 步、回報第 1 項都已改成「直接重跑即可，唯一還要回捲的是『抓到一些、漏掉另一些』那種情形」。其餘五處：①第 2 節「看 `last_run_utc` 是否推進到今天」這個判準已隨新行為失效（真 0 集的週末會被讀成 podfetch 掛了），已改成「一律看日誌」；②`healthcheck.py` 的 `last_run_utc` 新鮮度門檻同理，30 小時 → 96 小時並改寫訊息，否則每個 0 集的週末都會誤報 WARN；③重播規則只寫進 SKILL.md、brief 端仍只有 Money Stuff 那一格的個別敘述，**而第 8 節卻已經宣稱「兩邊都改了」**，現已在第 4 節立為全庫通則；④本節下方「`capitalallocators` 會走預設藍」與新寫的「26 組全部有 CSS」矛盾；⑤`MAINTENANCE.md` 第 4 節的 CSS 組數還是舊的 24。
