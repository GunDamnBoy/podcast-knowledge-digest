...
每天早上（含週末）自動偵測 20 檔 Podcast／YouTube 節目的新集數，取得**全文**（官方逐字稿或 YouTube 字幕），為每一集撰寫約 2,000–3,000 字的繁體中文完整摘譯＋3–5 個核心重點，同時交付兩種形式：
...
**檔名慣例**：`data/YYYY-MM-DD.json` 的日期是**執行當天**（台北），內容是該執行時點往前推 26 小時窗口內發布的集數——所以 `2026-07-30.json` 裝的是 7/29 晚間至 7/30 上午發布的集數。補跑歷史某天時要沿用同一慣例，否則日期會對不上。
...
**流程**：iTunes 偵測 → 下載 MP3 → 切成 **20 分鐘**段（見下）→ 每段經 **Files API** 上傳後送 Gemini `generateContent` → 合併 → 字數檢查 → 寫出 `.md` 與 `manifest.json`。

> 2026-08-03 更正：本行原寫「25 分鐘段／base64 內嵌／不需 File API」，是 08-02 最初版本的做法，當天稍晚就已被下文的「20 分鐘」與「Files API」取代（`config.json` 現值 `segment_seconds: 1200`、`max_chunk_mb: 48`），但這一行漏改，成了與同節下文直接打架的過期敘述。

**切段方式（2026-08-02 改版）**：原本用 ffmpeg，但使用者的 Mac 沒有 Homebrew，為了一個切檔動作要求安裝套件管理器不合理。改為**用純 Python 解析 MPEG Layer III 的 frame 標頭、在 frame 邊界切檔**，不重新編碼，因此零外部相依。單段同時受「秒數」與「檔案大小」兩個上限約束，高位元速率的檔案會自動切得更短。**大小上限現為 `max_chunk_mb: 48`**——最初的 9 MiB 是 inline base64 時代的限制（膨脹約 1.34 倍須低於 20MB 請求上限），改走 Files API 後該限制已不適用。第一個 frame 若是 Xing／Info／VBRI 標頭會跳過——它記的是整個原檔的長度，複製到第一段會讓解碼器誤判時長。若系統剛好有 ffmpeg 則優先使用（會順便降到 16kHz 單聲道 32kbps，上傳量小很多），失敗時自動退回內建切檔器。
...
新版 Flash-Lite 的日額度是一般 Flash 的 **25 倍**，這是整個設計裡最重要的一個數字。模型池因此固定為「3 個 Flash（品質優先）＋ 3 個 Flash-Lite（溢流）」，合計逾 1,000 RPD，而一天只需約 22 個請求（20 分鐘段長下的實際值；本行原寫 15，是 25 分鐘段時代的估算）。**不要把 Lite 名額拿掉**，那等於自願放棄 25 倍額度。另註：`gemini-flash-latest` 目前對應 Gemini 3.6 Flash，屬 20 RPD 那一組；`gemini-2.5-flash` 已對新專案關閉（呼叫回 404），`gemini-2 flash`／`gemini-3.1 pro`／`gemini-2.5 pro` 在免費層是 0/0 不開放。
...
**排查時務必看日誌的時間戳，不是只看內容（2026-08-03 事故，已定案）。** 08-03 的日誌開頭是 `[07:00:01]` 而非 `[01:00:0x]`，比日報的 03:00 還晚四小時。

**根因不是排程設定，是機器在睡覺。** 當天實際查證的順序值得記下來，因為第一個假設是錯的：一開始從日誌時間戳推論「plist 被設成 07:00」，但實際印出 `StartCalendarInterval` 是 `Hour = 1, Minute = 0`，一直都對。真正的原因是 `pmset -g sched` 完全空白——**沒有任何排定的喚醒事件**。`StartCalendarInterval` 不會喚醒 Mac，01:00 機器睡著時 launchd 就是不跑，要等下次醒來才補跑一次。07:00 那個「整點又剛好 01 秒」正是喚醒瞬間補跑的樣子。旁證：`~/.podfetch/logs/` 當時只有 08-02（手動 19:29）與 08-03 兩份，**08-03 07:00 是 launchd 有史以來第一次自動觸發，第一次就沒準時**。

**教訓是「時間戳對不上」有兩種完全不同的成因，不要跳過第一種驗證就下結論**：(a) plist 被改動；(b) 機器睡眠導致延後補跑。先 `PlistBuddy -c "Print :StartCalendarInterval"` 看設定、再 `pmset -g sched` 看有沒有喚醒排程，兩者分得開。

後果不因成因而減輕：日報 cron 改成 03:00 後，只要 podfetch 當天延後補跑，日報就會在它之前執行，於是**讀不到當天目錄、也讀不到當天日誌，被上面第 3 點誤判成「podfetch 失效」而掉進已死的 YouTube 退援路徑，產出降級內容並誤報故障**。08-03 沒有爆掉純粹是因為日報那次仍跑在舊的 09:00。修法見第 6 節「排定喚醒」。
...
- **排定喚醒 `pmset`（2026-08-03 新增，整套系統的隱藏前提）**：launchd `com.kenny.podfetch` 設在 01:00、日報設在 03:00，但**這兩個排程都不會把睡著的 Mac 叫醒**。`StartCalendarInterval` 錯過就等下次醒來補跑一次；Cowork 排程同理（App 關閉時錯過的會在下次啟動補跑）。08-03 首次自動觸發就延到 07:00，根因即為 `pmset -g sched` 全空、沒有任何喚醒排程。

  已設定：`sudo pmset repeat wakeorpoweron MTWRFSU 00:55:00`，即每天 00:55 自動喚醒，留 5 分鐘給 01:00 的 podfetch。**驗證**：`pmset -g sched` 應印出 `wakepoweron at 0:55AM every day`。

  **兩個限制要記住**：(1) 排定喚醒在 **接電源時最可靠**，只靠電池時 macOS 可能略過；(2) 從**完全關機**狀態喚醒，筆電基本不會成功（`poweron` 主要對桌機有效）。因此「晚上關機」或「不插電」都會讓 01:00／03:00 這條時序回到延後補跑的狀態。若使用習慣改變，就不能再靠時序保證，得改成讓日報容忍 podfetch 尚未跑完。

  重灌系統或換機時，`pmset repeat` **不會**跟著遷移，要記得重設——這是唯一一個不存在於任何設定檔裡的依賴。

- **podfetch 排程時刻**：launchd `com.kenny.podfetch` 必須在 **01:00**，比日報的 03:00 早兩小時。`~/.podfetch/fix-schedule.sh` 可把 plist 的 `StartCalendarInterval` 寫回 01:00 並重新 `launchctl load`，08-03 執行時確認設定本來就正確（真正的原因是上面的喚醒問題），因此該腳本現在的用途是**驗證與還原**，不是例行修復。**每日驗證方式**：看 `~/.podfetch/logs/<日期>.log` 開頭時間戳是否為 `[01:00:0x]`。腳本刻意放在 `~/.podfetch/`（repo 外部），不要移進 repo。
...
- **修正 podfetch 與日報的時序倒置（本次最重要的一項）**。巡檢發現 launchd `com.kenny.podfetch` 實際跑在 **07:00** 而非 brief 與 SKILL.md 都寫的 01:00。當天沒有出事，只是因為日報那次仍跑在改制前的 09:00；cron 一改成 03:00，日報就會每天早於 podfetch 執行，**讀不到當天目錄與日誌 → 被三段排查的第 3 點誤判成「podfetch 失效」→ 掉進已死的 YouTube 退援路徑**。這個失效模式與 8/2 的 `auto-push.sh` 事故同一種：元件本身完全正常，只有品質悄悄降級，沒有任何外顯徵兆。
- **根因是「機器在睡覺」，不是排程設定——第一個假設是錯的，值得記下修正過程。** 從日誌時間戳一開始推論「plist 被設成 07:00」，實際印出 `StartCalendarInterval` 卻是 `Hour = 1, Minute = 0`，設定一直正確。真正的原因是 `pmset -g sched` 全空，沒有任何排定喚醒；`StartCalendarInterval` 不會喚醒 Mac，01:00 睡著就是不跑，等醒來補跑一次。**修法是 `sudo pmset repeat wakeorpoweron MTWRFSU 00:55:00`**（見第 6 節），不是改 plist。教訓：「時間戳對不上」至少有兩種成因，先驗設定、再驗喚醒，不要看到症狀就跳到結論。
- **並在第 2 節加入「時刻漂移」守衛**：排查時不能只看日誌內容，要順手確認開頭時間戳是 `[01:`；不是就代表當天 podfetch 延後了，當日回報必須明講。單靠文件寫「01:00」擋不住這件事，得讓每天的執行者實際去看一眼。
- **`pmset` 排定喚醒登記為基礎設施依賴**（第 6 節）。它是整套系統裡**唯一不存在於任何設定檔的依賴**——重灌或換機不會跟著遷移，且在「電池供電」與「完全關機」兩種情況下會失效。
...
- **修掉本檔內部四處自打架的過期數值**（都是 08-02 當天邊做邊改留下的殘跡，下文已更新、上文漏改）：第 2 節流程行的「25 分鐘段／base64 內嵌／不需 File API」改為「20 分鐘段／Files API」；切段的「9 MiB」上限改為 `max_chunk_mb: 48`（9 MiB 是 inline base64 時代的限制）；「一天約 15 個請求」對齊為約 22 個；第 0 節「每個平日」改為「每天（含週末）」，第 1 節檔名慣例殘留的「26／74 小時」改為 26 小時（74 小時的週一規則已於 08-02 廢止）。
- **補上排程 SKILL.md 缺的四條 brief 規則**：(1) manifest 決定「有哪幾集」而非「用哪份全文」——A 類節目仍須抓官方稿，原本 SKILL.md 的「不要開 Chrome」會讓官方稿永遠拿不到；(2) podfetch 主路徑同樣要對前一天的 json 去重（原本只寫在 iTunes 退援那一步，而 26 小時窗口的 2 小時重疊在主路徑一樣會重複命中）；(3) 寫不進 repo 時的退援是「照常交付並附上 json 請使用者手動放置」，不是放棄當天產出；(4) YouTube 須為已登入狀態、該集未上架 YouTube 的處理、每集保留來源連結與付費來源只做濃縮的合規要求。
