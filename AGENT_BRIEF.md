...
- **US 商店快取嚴重過期**（尤其 All-In），且 **limit 越小快取越舊**。**第一線解法是上面的 `&cb=<時間戳>`**——過期是綁在查詢字串上的，換任何一個變動參數就繞開。2026-08-15 實測（台北 03:50 那次查詢）：不帶 `cb` 時 All-In 與 Bloomberg 都停在 07-31（落後 15 天），帶了 `cb` 立刻拿到 All-In 08-13、Bloomberg 08-14。**引用這類實例時要連查詢時點一起寫**——同一支 AppleID 在幾小時後就會多出新集數（All-In 那集 08-14T20:11Z 發布，早上再查就變成最新一筆），沒有時點的數字日後無法核對。**若加了 `cb` 仍然可疑，才換商店，而且要用查詢參數形式 `&country=GB`——路徑前綴形式（`itunes.apple.com/gb/lookup`）在本環境回空白內容，已不可用**（2026-08-15 實測，brief 舊版寫的 `/gb/`／`/au/` 是錯的）。**這一條若漏掉，主路徑與退援路徑都會安靜漏抓主秀**——08-15 就這樣整整一天回報「沒有新集數」，實際有 10 集。
...
另有 `overload_cooldown_seconds: 300`（500／503 的冷卻秒數，見上）、`episode_budget_seconds: 3600`（單集牆鐘上限）、`run_budget_seconds: 6000`（整場執行上限）、`stale_feed_hours: 72`（0 集健全性對照門檻，見下）與 `prefer_lite: true`（模型池是否 Lite 優先）。**後者是布林值，`healthcheck.py` 的數值比對抓不到它**，改動時要自己確認 `config.json` 與本節一致。
...
> `capitalallocators` 於 2026-08-03 移除、`bg2` 與 `breakdowns` 於 2026-08-16 移除。**三者的 CSS 都留著，舊資料檔顯示正常，不會走預設藍**（這句在 08-16 之前寫的是「會走預設藍」，那是還沒補 CSS 時的狀態，已不成立）。**不要為此回頭改歷史檔案**，歷史資料保持原樣。
...
- **重播規則從 FT 專屬一般化成全庫通則。** 原本只有 Unhedged 那一格寫「feed 可能放姊妹節目重播」，而 Money Stuff 會發 `Re-Run:` 前綴的重播（08-14 那集是 3 月訪談 Boaz Weinstein）。現在 SKILL.md 第 2 步獨立成一點：主代理負責認出重播、任務卡的 `published` 填**原始播出日**、`source` 寫明是重播，回報第 14 項要列出來。**這是內容規格，brief 第 1 節與任務卡兩邊都改了。**
...
>
> **更早的變更紀錄（2026-08-02 至 08-15）已歸檔至 `MAINTENANCE.md` 第 11 節。**
