...
- `episodeCount`、`index.json` 的 `shows`、**以及交叉觀察的「當日 ≥3 集」門檻（第 4 節）**，一律按**去重後**的集數計算。
- 在回報中列出被合併的集數。
...
- **新增 We Study Billionaires（TIP），節目數 22 → 23。** 加它的唯一理由是**週末補檔**：週一那一版涵蓋美東週六中午到週日中午，現有 22 檔在這個窗口幾乎沒有供給（08-10 只有 2 集，還是同一支預告）。TIP 一週兩檔、**其中一檔固定在週日**（實查 08-09／08-02／07-26 皆為週日發布，UTC 00:00 ＝ 美東週六 20:00，**落在窗口內**）。四處都已補：brief 第 1 節、`shows.json`、`config.json` 的 `show_priority`（插在 `iltb` 之後）、`index.html` 三組 CSS（`#a16207`，唯一相近的是**已下架、不會出現在現行資料裡**的 `capitalallocators`）。**語速未校準**，先沿用全域 200，累積幾集後再決定要不要補 `wpm`。
