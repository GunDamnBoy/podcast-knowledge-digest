...
>
> 轉錄 prompt 已改為保守標記——有明確依據（自我介紹、被直呼、主持人點名）才寫真名，否則一律 `Speaker N`，廣告與台呼標 `Announcer`。**因此看到 `Speaker N` 是誠實而非失敗**，不要硬去補人名。可疑訊號由 `podfetch.py` 寫進**獨立的 `speakerNotes` 欄位（不是 `warnings`、不影響 `status`）**，判定條件見 `MAINTENANCE.md` 第 4C 節。
>
...
>
> 1. **時間戳溢出**——最大時間戳超過片長 25% 以上。**這種集數的完整度與 status 可能完全正常**（實例是 0.91／0.98、都是 OK），時間軸崩壞本身不會觸發任何其他檢查。
...
>
> **第 2 條刻意採「雙訊號」**——兩個獨立訊號同時成立才報。長篇獨白（時間戳只在換人時出現）與真的跳段長得一模一樣，只看間隙會對 72 集報 15 集，那就是講者檢查初版「九集報八集」的同一種失敗。收緊後全庫 72 集只報 7 集。**凡是單獨看必然帶雜訊的啟發式訊號，一律用雙訊號**（時間軸這條、`healthcheck.py` 的 0 集健全性對照都是；反面例子是比對兩個 ISO 日期字串那種無雜訊來源的檢查，那種單訊號就夠）。
>
...
- **以 `writing-for-agents` 六槓桿稽核本檔與排程 `SKILL.md`，分五批改（本檔 34,381 → 29,641 字元）。** 刪五處會過期的抄本（舊的工具呼叫預算數值、三處「宣告舊規則作廢卻把它完整寫進 context」的句子、23 組 AppleID 抄本、開場白）；修 `snapshot.sh` 在 Mac bash 3.2 上的解析錯誤；本節歸檔；**把 podfetch 內部設計與沿革敘述下推到 `MAINTENANCE.md` 第 4C／7／11 節**——**判準是逐句問「每日執行者會不會用到」**，執行者不執行 podfetch，只讀它的輸出。`config.json` 現行值刻意留著：它是 `healthcheck.py`「brief vs config」唯一的比對對象。
  **`SKILL.md` 同步**：第 0／1／2 步各補一條可驗證的完成條件、回報新增第 17 項（brief 讀了幾次），並引進「**進場**」一詞取代原本反覆展開的成本說明。本檔第 3 節另引進「**雙訊號**」（`SKILL.md` 端未引進，該處仍是展開式敘述）。
  **為什麼要瘦**：**本檔一次 `Read` 讀不完就會被截斷**（上限約 25,000 token），而第 0 步要求整份讀完。逐場量測與下一批候選見 `MAINTENANCE.md` 第 6 節。
