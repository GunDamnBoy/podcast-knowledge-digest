...
- **找到 08-11 那條「先估篇幅再落筆」失效的根因：尺是浮動的。** 08-12 五集仍有三集重寫（與前一天同樣是 3/5），成本 274 萬 vs 288 萬持平，但逐字稿少了 25%——**每 KB 成本反而上升**。實測全庫 13 個日檔 74 集，子代理自報的 `chars` 與正文實際字元數：**偏差中位數 0%，範圍 −38% 到 +81%，42% 的集數偏差 20% 以上**。這不是系統性偏差而是**雜訊**——一把無偏但誤差 ±40% 的尺，比一把有偏但穩定的尺更沒用。當天實例：Dwarkesh 自報 5,900／實際 8,709、Pivot 自報 5,948／實際 8,045（兩集都自以為在區間內、實際超上界近三成），而 In Good Company 自報 1,789／實際 1,087（自以為超標所以壓縮了兩輪，實際一直低於下界）。**在一把會浮動的尺上「先估篇幅」是不可能執行的。**
