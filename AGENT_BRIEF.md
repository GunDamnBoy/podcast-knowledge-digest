...
- **完整摘譯——依節目長度分層（2026-08-06 改）** — 依章節時間軸分節，忠於原文論點與數字，正反方意見都要呈現。
  ...
  > **先估篇幅再落筆，不要寫完再壓縮（2026-08-11 新增，這是成本規則不只是文風規則）。** 動筆前先做兩件事：①依片長查出目標字數；②先列出章節骨架並**分配每節字數**，加總落在區間內才開始寫。
...
- **任務卡加「先估篇幅再落筆」（第 3 節同步）。** 08-11 四集超出呼叫預算，主因是三集初稿超上界後重寫（Odd Lots 8,257→6,434、Acquired 14,255→6,583、Bloomberg 3,642→2,983）——**每重寫一輪，整份逐字稿就被重新計費一次**。寫作規格與成本預算在長節目上會互相打架，解法是先列骨架並分配字數，不是事後刪。理想呼叫數同步從 2 次改為 3 次（Read → 想骨架 → Write）。
