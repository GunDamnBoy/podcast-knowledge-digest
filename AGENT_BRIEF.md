...
- **以 `writing-for-agents` 六槓桿稽核本檔與排程 `SKILL.md`，並依稽核結果分批改（本檔 34,738 → 32,989 字元）。** 本場動了三批：①**刪五處會過期的抄本**——本檔第 5 行的舊工具呼叫預算數值（**唯一殘留的舊值，數值以 `SKILL.md` 為唯一來源**）、`SKILL.md` 開頭與本檔第 2 節共三處「為了宣告舊規則作廢而把它完整寫進 context」的句子、第 1 節 23 組 AppleID 抄本（**改指向 `~/.podfetch/shows.json`；`healthcheck.py` 不讀那份清單，退援路徑取 AppleID 的地方已在 `SKILL.md` 第 1 步就地註明**）、第 3 行的純敘述開場白。②**修 `snapshot.sh` 在 Mac 上的解析錯誤**（`${DEST}` 加大括號；bash 3.2 會把緊接的全形括號吃進變數名）。③**本節歸檔**——見下方歸檔規則的單位訂正。
  **稽核找到的最大一項尚未處理**：**本檔一次 `Read` 讀不完（實測 29,112 token > 25,000 上限）**，所以排程第 0 步「完整讀過 `AGENT_BRIEF.md`」在機制上做不到單次完成。**這與本日「`Read` 上限是 token 不是行數」是同一個機制，當時只套用到逐字稿、沒有回頭套用到本檔自己。** 待辦與下推候選見 `MAINTENANCE.md` 第 6 節。
