...
- **以 `writing-for-agents` 六槓桿稽核本檔與排程 `SKILL.md`，分五批改（本檔 34,381 → 29,641 字元）。** 刪五處會過期的抄本（舊的工具呼叫預算數值、三處「宣告舊規則作廢卻把它完整寫進 context」的句子、23 組 AppleID 抄本、開場白）；修 `snapshot.sh` 在 Mac bash 3.2 上的解析錯誤；本節歸檔；**把 podfetch 內部設計與沿革敘述下推到 `MAINTENANCE.md` 第 4C／7／11 節**——**判準是逐句問「每日執行者會不會用到」**，執行者不執行 podfetch，只讀它的輸出。`config.json` 現行值刻意留著：它是 `healthcheck.py`「brief vs config」唯一的比對對象。
  **`SKILL.md` 同步**：第 0／1／2 步各補一條可驗證的完成條件、回報新增第 17 項（brief 讀了幾次），並引進「**進場**」與「**雙訊號**」兩個詞取代原本反覆展開的說明。
...
> **單位是「一場」不是「一天」（2026-08-17 ② 訂正）**：一天跑兩場以上已是常態（08-16 有六場、08-17 有三場），照日期比會讓同日前一場的條目躲過歸檔，**本節於是只增不減**。
