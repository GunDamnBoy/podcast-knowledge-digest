...
- **新增「同日同源去重」（第 2 節）。** 08-10 當天兩集是**同一支 113 秒的〈Our Town〉預告**，分別被 Bloomberg Surveillance 與 Odd Lots 推送——**同一天、不同 url、不同中譯標題**，三個條件全部避開了現行去重。而唯一能認出同源的兩個欄位（manifest 原文 `title` 與 `durationMs`）**剛好都不在比對範圍內**。新規則比對這兩個欄位、兩者都相同才合併，保留 `show_priority` 較前者並在其 `meta` 加「同源推送」。**失敗方向刻意選在漏判那一側**：多留一集只是重複，誤刪會弄丟內容。
...
- **統一「26 小時」的說法（第 2 節）。** 原本第 2 節寫「固定 26 小時」、第 2 節的設計原則又寫「以 `last_run_utc` 為準，不是固定窗口」——**字面互相否定**。實測 08-10 的窗口是 24.5 小時，兩種描述都不吻合。現在一律以 `last_run_utc` 為準，26 小時只當說明用的近似值。
