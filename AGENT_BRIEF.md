...
> **未完成，需要人工接手**：第 1 節第 ① 點對應的 `SKILL.md` 第 4 步改動**尚未套用**——維護端讀不到排程正本（`~/Documents` 是 macOS TCC 保護區），只能由使用者整段貼上。**第 5.5 步當晚已確認確實在正本裡**（原先的疑慮是快照落後一輪造成的，SKILL.md 正本已以 `2026-08-16-233000` 補存快照），見 `MAINTENANCE.md` 第 4／6 節。
