...
2. **連上了但沒有今天的目錄** → 讀 `~/.podfetch/logs/<今天>.log`。停在異常處就是 podfetch 失效。也可看 `state.json` 的 `last_run_utc` 是否已推進到今天。
   ...
   > - 有集數落在視窗內卻沒被 podfetch 收到 → **假零集**。缺口若不處理會變成永久，處理方式是把 `last_run_utc` 回捲到前一次的值再重跑 podfetch，並在回報中明講。
   >
