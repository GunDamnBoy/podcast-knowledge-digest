...
另有 `overload_cooldown_seconds: 300`（500／503 的冷卻秒數，見上）、`episode_budget_seconds: 3600`（單集牆鐘上限）、`run_budget_seconds: 6000`（整場執行上限）、`stale_feed_hours: 72`（0 集健全性對照門檻，見下）與 `prefer_lite: true`（模型池是否 Lite 優先）。**`prefer_lite` 與 `avoid_preview_models` 都是布林值，`healthcheck.py` 的數值比對抓不到**（2026-08-17 補上後者），改動時要自己確認 `config.json` 與本節一致。**連同下面兩個清單型設定，需要人工比對的一共四項。**
...
- **完整摘譯——依節目長度分層（2026-08-06 改）** — 依章節時間軸分節，忠於原文論點與數字，正反方意見都要呈現。
  ...
  > **為什麼寫進規格**：連兩天各有三集初稿超出上界後重寫，而**每重寫一輪，整份逐字稿就被重新計費一次**。**寫作規格與成本預算在長節目上會互相打架，解法是先規劃、不是事後刪。**（逐集的初稿／成稿字數見 `MAINTENANCE.md` 第 7 節。）
