...
- **`metrics.csv` 新增 `subagent_tokens_k` 欄。** 08-17 表面每集 985K、是 08-14 的 3.5 倍，看起來像大幅退化；拆開後 **2.1 倍來自逐字稿變長**（116 KB/集 vs 55）、其餘來自**固定成本攤到 3 集而非 12 集**，而**子代理每 KB 是 4.53 vs 4.27、等於沒變**。`eff_tokens_k` 是整場工作階段，混了固定開銷與一次性維護動作（當天「分析用量」本身就佔 14.2%），**除以集數得到的數字不可比**——08-15 也踩過同一個坑。探測腳本本來就印子代理的加權值，先前只是沒存。**要看效率就看 `subagent_tokens_k ÷ transcript_kb`。**
...
- **事故敘事下推：只搬了兩塊、約 500 字元，並訂正了前一天的估計。** 前一日估「第 1–7 節有 43% 是事故敘事」，那是**行層級的上界**；實際切分九個候選區塊只有 2,529 字元（6.8%），**其中七塊逐塊判斷後判定該留**——113 秒預告佔滿當天兩集、跳針 28,122 個「I」讓完整度反向、wpm 不決定 `DEGRADED`、503 併進 `EXHAUSTED` 的事故鏈、時間軸雙訊號的設計理由，**這些細節本身就是規則的動機，刪了規則就變成沒來由的規定**。只把「維護者才用得到的 meta 紀律」與「逐項的事後數字」移進 `MAINTENANCE.md` 第 7 節。
