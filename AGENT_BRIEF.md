...
- **完整摘譯——依節目長度分層（2026-08-06 改）** — 依章節時間軸分節，忠於原文論點與數字，正反方意見都要呈現。
  ...
  >
  > **為什麼要寫死**：08-12 實測 13 個日檔 74 集，子代理自報的 `chars` 與實際字元數**偏差中位數 0%，但範圍是 −38% 到 +81%，42% 的集數偏差 20% 以上**——不是系統性偏差，是**雜訊**。一把無偏但誤差 ±40% 的尺，比一把有偏但穩定的尺更沒用：08-12 的 Dwarkesh 與 Pivot 自以為在區間內、實際超上界 26–34%，而 In Good Company 自以為超標壓縮了兩輪、實際一直低於下界。**08-11 那條「先估篇幅再落筆」失效的根因就在這裡——尺是浮動的，估不了。**
  >
  ...
  > **先估篇幅再落筆，不要寫完再壓縮（2026-08-11 新增，這是成本規則不只是文風規則）。** 動筆前先做兩件事：①依片長查出目標字數；②**在心裡**列出章節骨架並**分配每節字數**（**不要寫成檔案**——那會多一次工具呼叫，等於用成本換整齊），加總落在區間內才開始寫。
  > **為什麼寫進規格**：08-11 五集有三集初稿超出上界後重寫（Odd Lots 8,257→6,434、Acquired 14,255→6,583、Bloomberg 3,642→2,983），而**每重寫一輪，整份逐字稿就被重新計費一次**——那天四集超出工具呼叫預算，主因就是這個。**寫作規格與成本預算在長節目上會互相打架，解法是先規劃、不是事後刪。**
  ...
  "episodes": [
    {
      ...
      「↑ `chars` 由組檔時機械計算覆寫，見下方說明；子代理填的值一律被蓋掉」
      "summary": "一句話總結…",
