...
| 08-10 ④ | **維護 skill 本身改版** | podcast-maintain skill（經 `save_skill` 覆寫；**它不在 snapshot.sh 涵蓋範圍**，版本只存在 Cowork 帳號端）、本檔 | skill 裡的過期清單直接造成了 08-10 ② 的 README 漏補（「四處」實為八步）；「連續三次」寫死的次數已腐化；缺這兩天新增的七個同步比對點與三條驗證教訓 | ⏳ 下一場維護即是驗證。**設計原則改為「指向單一事實來源，不再複製清單」**——skill 指向第 5 節步驟表與第 12 節登記簿，而不是自己再抄一份 |
| 08-11 | **首次量測驗收＋四項修正** | brief、SKILL.md、`index.html`、`data/observations.json`、本檔 | 呼叫預算與字數上界互相打架（三集寫完再壓縮）；acquired 無 CSS 實際撞到；記分板種子含免責雜訊；自報呼叫次數低報；超長節目不取官方稿無規格依據 | ✅ **本場是驗收不是修補**：08-08＋08-09 的 token 優化實測降 70%（970 萬→288 萬）、集層級時限實測 11 分 44 秒遠低於 60 分上限。四項修正待 08-12 驗證 |
...
- **方向對但成效未證實或被吃回去**：`08-03 ③`（⚠️，瘦身兩天內被變更紀錄吃回去）。
...
**這三類與 ⚠️ 標記不是一一對應的**，不要用「⚠️ 有幾列」去核對「發生幾次」——第一類有些列標的是 ✅（問題在後續那一列才被抓出來，本列的修法當下成立）。**改標記或加新列時，三個 bullet 都要重讀一次，而且改字與改表必須同一次做完。** 08-09 ②、08-10、08-11 **連續三輪**抓到這段列舉與表對不上：前兩次是只改表沒改字，第三次（08-11）反過來，**只改字沒改表**——宣稱「第一筆翻正」時表上其實一格都沒動。形態相同、方向相反。
