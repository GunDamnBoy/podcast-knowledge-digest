...
- **plist 改 `Hour = 1` 後尚未經過一次實跑（2026-08-03 傍晚改的）**：08-04 01:00 是第一次驗證。隔天早上看 `healthcheck.py` 的「podfetch 時刻」是否 PASS（日誌開頭應為 `[01:`）。
- **`pmset` 那組設定同樣待觀察**：若時間戳準時就代表兩邊都對；若仍偏晚且 plist 確認為 1，才是喚醒問題，多半是那晚沒接電源或關機了。連續失效就別再靠時序，改成讓日報容忍 podfetch 尚未跑完（例如短暫等待後重讀，或改用前一天的 manifest 並在回報中標明）。
