...
08-09 有兩集標 `DEGRADED`（完整度 1.44 與 1.53）。掃過**單 token 與片語級跳針都是 0**——不是跳針，是 All-In 那類四人交叉搶話的節目本來就講到 238 字/分。
...
| 08-08 ② | **token 優化** | SKILL.md（重寫）、`json2docx.py`（新）、brief（68.8→44 KB） | 逐字稿在主 context 被重複計費數十次；同一份內容 LLM 寫兩次 | ⚠️ **架構成立、降幅未證實**。08-09 實測：逐字稿確實一個字都沒進主對話（檔案操作僅 4.8%），但**五個子代理吃掉全程 84%**——成本從「一份大 context 被重讀很多次」變成「五份小 context 各自被重讀很多次」。好處是改為線性成長，集數多時才顯優勢。**當初「降 60–75%」是估算，至今無法驗證** |
...
**讀這張表的方式**：「後來成立嗎」那一欄的 ❌ 與 ⚠️ 是最有價值的部分——它們標出**修法本身製造新問題**的案例，目前已發生四次（08-04→08-06、08-03→08-03⑤、08-06①→08-06②、08-09①→08-09②）。動任何防護機制前先看這一欄。
