...
這不是規則寫壞，是**規則的立論範圍寫死了**：brief 原文明講「26 小時對 24 小時留有 2 小時重疊，代價是**相鄰兩天**會重複命中同一集」——去重從一開始就只為「跨日」設計，同日同源不在它的世界觀裡。**當一條規則附了它自己的理由，那個理由就是它的邊界；邊界外的同類問題不會觸發任何警訊。**
...
| 08-10 | 同源去重、下界例外、子代理門檻 | brief、SKILL.md、`healthcheck.py`、本檔 | 同一支音檔被兩個 feed 推送而佔滿當日；規格缺「低於下界」與「低量日」條款；token 量測讀錯路徑 | ⏳ 08-11 起驗證。**三個缺口共同形態是「主代理靠自覺補上規格的缺口」**，見第 7 節 |

**讀這張表的方式**：「後來成立嗎」那一欄的 ❌ 與 ⚠️ 是最有價值的部分，但**兩種標記代表兩件不同的事，不要混為一談**：

- **修法本身製造新問題**（動防護機制前一定要先看這幾筆）：`08-04`（❌，503 併進 `EXHAUSTED` → 08-06 事故）、`08-06 ①`（雖標 ✅，但當下缺輪換上限，由 `08-06 ②` 補）、`08-03 ③`（雖標 ✅，根因誤判由 `08-03 ⑤` 撤銷）、`08-09 ②`（⚠️，文件層三種副作用）。**四次。**
- **方向對但成效未證實或被吃回去**：`08-03 ③`（⚠️，瘦身兩天內被變更紀錄吃回去）、`08-08 ②`（⚠️，架構成立、降幅至今無法驗證）。

改標記時**要同時檢查這段列舉**——2026-08-09 ② 就抓到這句話列的四個案例與表上實際標 ⚠️ 的四列完全對不上，數字巧合相同才更難發現。
