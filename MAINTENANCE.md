...
**還查出一件更難堪的事**：昨天（08-09）才加的 token 量測，`eff_tokens_k` 連續兩天都是空的——`measure_session_tokens()` 讀 `~/.claude/projects`，那是 Claude Code CLI 的路徑，而每日排程跑在 Cowork，transcript 在 `$TMPDIR/claude-hostloop-plugins/<hash>/projects/` 底下。**加了量測卻沒量到，比沒加更糟**，因為 `metrics.csv` 已經有那三欄，看起來像正在累積基線。**寫量測時要順手驗證它真的回傳非空值**——這一項昨天做完就直接收工了。
...
| 08-10 | 同源去重、下界例外、子代理門檻、退援層號、26 小時統一、token 路徑 | brief、SKILL.md、`healthcheck.py`、本檔 | 同一支音檔被兩個 feed 推送而佔滿當日；規格缺「低於下界」與「低量日」條款；FT 被誤列成獨立的退援層；26 小時與 `last_run_utc` 字面互斥；token 量測讀錯路徑 | ⏳ 08-11 起驗證。**前三個缺口共同形態是「主代理靠自覺補上規格的缺口」**，見第 7 節 |
