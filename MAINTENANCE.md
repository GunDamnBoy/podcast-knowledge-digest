...
- **排查時要看日誌的時間戳，不是只看內容（2026-08-03）。** podfetch 必須早於日報：01:00 轉錄、03:00 日報。08-03 巡檢發現實際跑在 07:00，日報會讀不到當天目錄與日誌，被三段排查的第 3 點誤判成「podfetch 失效」而掉進已死的 YouTube 退援。
- **「時間戳對不上」有兩種成因，別跳過第一種驗證（2026-08-03，第一次診斷就錯在這）。** (a) plist 被改動 → `/usr/libexec/PlistBuddy -c "Print :StartCalendarInterval" ~/Library/LaunchAgents/com.kenny.podfetch.plist`；(b) **機器睡著、launchd 延後補跑** → `pmset -g sched` 看有沒有排定喚醒。08-03 的真相是 (b)：plist 一直是 01:00，但完全沒有喚醒排程。修法是 `sudo pmset repeat wakeorpoweron MTWRFSU 00:55:00`，不是改 plist。`~/.podfetch/fix-schedule.sh` 現在只是驗證／還原工具。
...
- **`pmset` 排定喚醒待觀察（2026-08-03 設定）**：接下來幾天早上看 `~/.podfetch/logs/<日期>.log` 開頭是否穩定為 `[01:00:0x]`。若仍偶爾延後，多半是那晚沒接電源或關機了。連續失效就別再靠時序，改成讓日報容忍 podfetch 尚未跑完（例如短暫等待後重讀，或直接改用前一天的 manifest 並在回報中標明）。
