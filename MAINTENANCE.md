...
| 08-08 ② | **token 優化** | SKILL.md（重寫）、`json2docx.py`（新）、brief（68.8→45 KB） | 逐字稿在主 context 被重複計費數十次；同一份內容 LLM 寫兩次 | ⚠️ **架構成立、降幅未證實**。08-09 實測：逐字稿確實一個字都沒進主對話（檔案操作僅 4.8%），但**五個子代理吃掉全程 84%**——成本從「一份大 context 被重讀很多次」變成「五份小 context 各自被重讀很多次」。好處是改為線性成長，集數多時才顯優勢。**當初「降 60–75%」是估算，至今無法驗證** |
...
- **原本未證實、後來被實測證明成立**：`08-08 ②`＋`08-09 ①` 這一組——架構隔離加上呼叫預算，**08-11 實測同樣五集 970 萬 → 288 萬（降 70%）**，落在當初粗估的 60–75% 區間內。`08-09 ①` 的 token 量測那半仍未生效（路徑猜錯兩次，08-10 改為失敗會出聲）。**這是登記簿上第一筆從 ⚠️ 翻正的紀錄，值得留著當對照**：優化的方向判斷對了，但要等到有量測才知道對在哪一段。

**這兩類與 ⚠️ 標記不是一一對應的**，不要用「⚠️ 有幾列」去核對「發生幾次」——第一類有些列標的是 ✅（問題在後續那一列才被抓出來，本列的修法當下成立）。**改標記或加新列時，兩個 bullet 都要重讀一次。** 2026-08-09 ② 與 08-10 連續兩次抓到這段列舉與表對不上，**而且兩次都是因為只改表沒改這段字**。
